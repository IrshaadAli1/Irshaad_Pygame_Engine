...
def _adjust_length(arr, factor):
    factor = factor ** -1
    if factor > 1:  # Lengthen the array (duplicate values)
        return [x for x in arr for _ in range(int(factor))]
    elif factor < 1:  # Shorten the array (take every nth element)
        return arr[::int(1 / factor)]
    else:  # No change if factor is exactly 1
        return arr
